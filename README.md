...
- **design-system** — design tokens (DTCG + Tailwind v4 `@theme`), OKLCH ramps, semantic theming, multi-brand (`brand-alt.tokens.json` ships as a worked cobalt+coral second brand) & dark mode, dev handoff spec sheets, and a `brand-lint` `undefined-token-ref` gate (a `var()` to a renamed token fails CI).
...
- **frontend-build** — design → working code: React/Next + Tailwind v4 + shadcn/ui, accessibility, responsive, screenshot/Figma → code, verify-by-screenshot. Ships a runnable **React + Vite + Tailwind v4** starter (`assets/starter-react/`, a cva `Button` + a stateful screen) and a flat-config `eslint.config.js` a11y preset.
...
- **`icon-set.js`** — emits a coherent first-party SVG icon family from a small spec (shared grid/stroke/caps, per-icon optical correction) + a self-contained `icon-grid.html` verification sheet (every icon at 16/24/32px on light + dark). No spec → a default core set of 12.
