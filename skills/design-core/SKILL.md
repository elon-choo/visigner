...
- **Auto-critique is the DEFAULT, not something you remember to run.** Producing an `*.html` design artifact auto-fires the anti-slop grade — the plugin's `PostToolUse` hook (`hooks/auto-critique-hook.js` → `anti-ai-eval`) runs by default, with no manual call and no way to silently skip it. Your job is to iterate to the ship gate, not to decide whether to grade. When a browser is present the hook renders, captures, and hands tiles off to design-critic; you run the bounded critique→iterate loop to the ship gate. When it is NOT, the static grade still runs and you MUST tell the user *pixel critique is OFF — run `/design-setup`* rather than report an ungraded page as "looks right." **Machine-clean is necessary, not sufficient:** a page can score 100/A and still read as AI, so carry the taste read (the taste jury / a human eye) and never treat the machine pass alone as done.
