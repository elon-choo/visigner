...
```
It checks **11 required sections** (problem, goals, non-goals, users, success-metrics, scope-in, scope-out, flows, acceptance-criteria, event-spec, open-questions) — detected from markdown **headings OR bold labels**, so a plan written either way passes — plus structural floors. `event-spec-table` (a real markdown table exists in the event-spec section, §7e), `flow-error-path` (every flow names an error/edge/unhappy path — now **also satisfied by a decision node with ≥2 labeled outcomes** in the §3 arrow notation, e.g. `card valid -> success / declined -> error`, 성공/실패, yes/no, even without the literal word "error"; the keyword set also gained expired/revoked/conflict/locked/duplicate/unauthorized/forbidden/cancelled + 만료/취소/중복/충돌), and `inventory-ac` (every screen/state in a "Screen/State Inventory" section is referenced in acceptance criteria; **no inventory section → WARN only**). **Two newer ERROR floors can newly FAIL a previously-passing plan** — each runs **only when its section exists**: `ac-gwt` (acceptance criteria must be written as **Given/When/Then** scenarios, §7 — a bare bullet list of requirements no longer passes) and `metric-shape` (every success-metric line — list item or table row — must carry **a number/target AND a named instrument** such as PostHog / funnel / dashboard / SQL / Amplitude, §1/§7e — so "improve engagement" fails). It prints a PASS/FAIL/WARN checklist and an optional JSON report; **exit 1** on any required/structural ERROR, 0 when the floor is met, 2 fatal. (Off-by-default — nothing imports it; run it explicitly.) Fix every ERROR, then hand the plan to the critic. (Note: the flows matcher still grabs an H1 containing the word "Flow" — pre-existing first-match behavior — so don't title the whole document "…Flow".)
