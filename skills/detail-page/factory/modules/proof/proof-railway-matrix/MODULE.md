# proof-railway-matrix

타입: `proof` · 규격: SECTION-SCHEMA `1.0.0` · 게이트: `section-motion-v1`

## 1. 왜 이 방식인가

증거를 짧은 matrix로 정리해 각 근거의 이름과 설명을 같은 읽기 단위에 둔다.

프로파일 해부 앵커: `claim` · `evidence-set` · `source-or-context`. railway의 high-confidence proof 구조와 그 레코드의 styles inventory를 함께 사용한다. 세부 카피와 브랜드 표현은 일반화하지 않는다. 기본값은 `grid` · `contrast-bleed`이며, 슬롯과 semantic 토큰으로만 일반화한다.
...
- 슬롯을 바꾸면 → 필수 슬롯 `headline`, `proofItems`와 `module.json`, template 앵커, SECTION-SCHEMA를 함께 검토해야 한다. 재검증: `node v3/scripts/section-validate.js v3/modules/proof/proof-railway-matrix` + `node v3/scripts/grammar-lint.js <composition.json>` + 시각 검토.
...
pending — 예약 경로: v3/renders/gold/proof--proof-railway-matrix/
...
- recordId: `railway`
- span order: `9`
- sectionSpan: `captures/stage2/railway/tile_04.png#proof`
