...
증거의 제목과 근거 집합을 두 검토 면으로 나눠 핵심 주장과 세부 항목을 구별한다.

프로파일 해부 앵커: `claim` · `evidence-set` · `source-or-context`. supabase의 high-confidence proof 구조와 그 레코드의 styles inventory를 함께 사용한다. 세부 카피와 브랜드 표현은 일반화하지 않는다. 기본값은 `split` · `media-bleed`이며, 슬롯과 semantic 토큰으로만 일반화한다.
...
- 슬롯을 바꾸면 → 필수 슬롯 `headline`, `proofItems`와 `module.json`, template 앵커, SECTION-SCHEMA를 함께 검토해야 한다. 재검증: `node v3/scripts/section-validate.js v3/modules/proof/proof-product-brief` + `node v3/scripts/grammar-lint.js <composition.json>` + 시각 검토.
...
pending — 예약 경로: v3/renders/gold/proof--proof-product-brief/
...
- recordId: `supabase`
- span order: `14`
- sectionSpan: `captures/stage2/supabase/tile_01.png#proof`
