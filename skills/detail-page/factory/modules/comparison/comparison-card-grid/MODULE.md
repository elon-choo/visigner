...
공통 기준 행을 카드형 grid 안에 보존해 비교 기준이 선택지 간에 흩어지지 않게 한다.

프로파일 해부 앵커: `comparison-title` · `shared-criterion-rows` · `interpretation-aid`. comparison의 구조 스팬은 없으므로 SECTION-SCHEMA의 비교 제목·공통 기준 행·해석 보조 계약만으로 작성한다. 기본값은 `grid` · `card-contained`이며, 슬롯과 semantic 토큰으로만 일반화한다.
...
- 슬롯을 바꾸면 → 필수 슬롯 `headline`, `comparisonItems`와 `module.json`, template 앵커, SECTION-SCHEMA를 함께 검토해야 한다. 재검증: `node v3/scripts/section-validate.js v3/modules/comparison/comparison-card-grid` + `node v3/scripts/grammar-lint.js <composition.json>` + 시각 검토.
...
pending — 예약 경로: v3/renders/gold/comparison--comparison-card-grid/
