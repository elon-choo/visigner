# comparison-split-ledger

타입: `comparison` · 규격: SECTION-SCHEMA `1.0.0` · 게이트: `section-motion-v1`

## 1. 왜 이 방식인가

비교 제목과 기준 행을 두 읽기 면으로 나눠 해석을 돕되 실제 제품 우열을 주장하지 않는다.

프로파일 해부 앵커: `comparison-title` · `shared-criterion-rows` · `interpretation-aid`. comparison의 구조 스팬은 없으므로 SECTION-SCHEMA의 비교 제목·공통 기준 행·해석 보조 계약만으로 작성한다. 기본값은 `split` · `contrast-bleed`이며, 슬롯과 semantic 토큰으로만 일반화한다.
...
- 슬롯을 바꾸면 → 필수 슬롯 `headline`, `comparisonItems`와 `module.json`, template 앵커, SECTION-SCHEMA를 함께 검토해야 한다. 재검증: `node v3/scripts/section-validate.js v3/modules/comparison/comparison-split-ledger` + `node v3/scripts/grammar-lint.js <composition.json>` + 시각 검토.
...
pending — 예약 경로: v3/renders/gold/comparison--comparison-split-ledger/
