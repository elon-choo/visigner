# cta-stack-invitation

타입: `cta` · 규격: SECTION-SCHEMA `1.0.0` · 게이트: `section-motion-v1`

## 1. 왜 이 방식인가

제목·설명·행동을 짧은 세로 흐름으로 쌓아 다음 단계의 순서를 명확히 한다.

프로파일 해부 앵커: `action-title` · `short-explanation` · `primary-action`. cta의 실제 구조 스팬을 과장하지 않고 SECTION-SCHEMA의 행동 제목·설명·주 CTA 계약으로 작성한다. 기본값은 `stack` · `media-bleed`이며, 슬롯과 semantic 토큰으로만 일반화한다.
...
- 슬롯을 바꾸면 → 필수 슬롯 `headline`, `body`, `primaryCtaLabel`, `primaryCtaHref`와 `module.json`, template 앵커, SECTION-SCHEMA를 함께 검토해야 한다. 재검증: `node v3/scripts/section-validate.js v3/modules/cta/cta-stack-invitation` + `node v3/scripts/grammar-lint.js <composition.json>` + 시각 검토.
...
pending — 예약 경로: v3/renders/gold/cta--cta-stack-invitation/
...
- sectionSpan: `v3/SECTION-SCHEMA.md#cta`
