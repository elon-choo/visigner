...
짧은 행동 문장을 가로 읽기 폭에 배치해 설명과 CTA를 독립 단위로 검토하게 한다.

프로파일 해부 앵커: `action-title` · `short-explanation` · `primary-action`. cta의 실제 구조 스팬을 과장하지 않고 SECTION-SCHEMA의 행동 제목·설명·주 CTA 계약으로 작성한다. 기본값은 `rail` · `surface-bleed`이며, 슬롯과 semantic 토큰으로만 일반화한다.
...
- 슬롯을 바꾸면 → 필수 슬롯 `headline`, `body`, `primaryCtaLabel`, `primaryCtaHref`와 `module.json`, template 앵커, SECTION-SCHEMA를 함께 검토해야 한다. 재검증: `node v3/scripts/section-validate.js v3/modules/cta/cta-rail-prompt` + `node v3/scripts/grammar-lint.js <composition.json>` + 시각 검토.
...
pending — 예약 경로: v3/renders/gold/cta--cta-rail-prompt/
