...
탐색 묶음 뒤에 하나의 명확한 연락 경로와 법적 행을 쌓아 말단의 읽기 순서를 짧게 유지한다. rail은 항목 간 진행 방향을 만들므로, 동등한 법적·탐색 정보를 다루는 이 변형에서는 배제했다. 카탈로그에서 압축형 legal footer-band 역할을 맡는다.

출처 타일에서 확인한 범위: 작은 탐색 열과 분리된 법적 행이 같은 말단 면에 놓인 간결한 구조를 관찰했다. 기본값은 `stack` · `card-contained`이며, 원본의 카피·브랜드·픽셀 값을 옮기지 않고 슬롯과 semantic 토큰으로 일반화했다.
...
- (c) 다른 모듈: 없음(독립). 조성 시 같은 `footer-band`·`stack` 조합은 한 번만 둘 수 있다.
...
- `layoutArchetype` 또는 `backgroundBleed` 기본값을 바꾸면 → 해당 CSS의 grid/표면 관계와 조성의 인접 다양성·이음새 규칙을 함께 바꿔야 한다. 재검증: `node v3/scripts/section-validate.js v3/modules/footer-band/compact-legal-footer` + `node v3/scripts/grammar-lint.js <composition.json>` + 시각 검토.
...
pending — 예약 경로: v3/renders/gold/footer-band--compact-legal-footer/

## 6. 출처

- recordId: `clerk`
- span order: `40`
- sectionSpan: `captures/stage2/clerk/tile_04.png#footer-band`
- 보조 구조 관찰: recordId: `kr-detail-29cm-mongdol-2530940` · span order: `26` · sectionSpan: `captures/kr-detail-29cm-2530940-r3/tile_38.png#footer-band`
