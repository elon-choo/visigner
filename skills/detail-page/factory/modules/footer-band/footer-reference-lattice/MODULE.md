# footer-reference-lattice

타입: `footer-band` · 규격: SECTION-SCHEMA `1.0.0` · 게이트: `section-motion-v1`

## 1. 왜 이 방식인가

연락 경로와 탐색 묶음을 짧은 reference lattice로 정리해 말단 탐색의 폭을 제한한다.

프로파일 해부 앵커: `navigation-set` · `contact-route` · `legal-copy`. arc의 high-confidence footer-band 구조와 그 레코드의 styles inventory를 함께 사용한다. 원본 링크·카피·브랜드 표현은 일반화하지 않는다. 기본값은 `rail` · `media-bleed`이며, 슬롯과 semantic 토큰으로만 일반화한다.

## 컬렉션 아이템 구조

- `navItems`의 항목은 `{ "groupLabel": string, "links": [{ "label": string, "href": string }] }`이다. 렌더러는 navItems 컨테이너 안에 항목 하나당 li 하나를 만든다.
...
- 슬롯을 바꾸면 → 필수 슬롯 `navItems`, `legalText`와 `module.json`, template 앵커, SECTION-SCHEMA를 함께 검토해야 한다. 재검증: `node v3/scripts/section-validate.js v3/modules/footer-band/footer-reference-lattice` + `node v3/scripts/grammar-lint.js <composition.json>` + 시각 검토.
...
pending — 예약 경로: v3/renders/gold/footer-band--footer-reference-lattice/
...
- recordId: `arc`
- span order: `4`
- sectionSpan: `captures/stage2/arc/tile_03.png#footer-band`
