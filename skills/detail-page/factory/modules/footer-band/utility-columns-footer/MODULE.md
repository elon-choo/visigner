# utility-columns-footer

타입: `footer-band` · 규격: SECTION-SCHEMA `1.0.0` · 게이트: `section-baseline-v1`

## 1. 왜 이 방식인가

짧은 utility·연락 영역과 탐색 열을 두 영역으로 분리해, 말단 행동이 링크 집합에 묻히지 않게 한다. grid만의 동등 카드화는 utility의 보조 역할을 과장하므로 배제했다. 카탈로그에서 utility 우선 footer-band 역할을 맡는다.

출처 타일에서 확인한 범위: 말단의 별도 utility 영역과 복수 탐색 열, 법적 행이 분리된 구조를 관찰했다. 기본값은 `split` · `surface-contained`이며, 원본의 카피·브랜드·픽셀 값을 옮기지 않고 슬롯과 semantic 토큰으로 일반화했다.
...
- (c) 다른 모듈: 없음(독립). 조성 시 같은 `footer-band`·`split` 조합은 한 번만 둘 수 있다.
...
- `layoutArchetype` 또는 `backgroundBleed` 기본값을 바꾸면 → 해당 CSS의 grid/표면 관계와 조성의 인접 다양성·이음새 규칙을 함께 바꿔야 한다. 재검증: `node v3/scripts/section-validate.js v3/modules/footer-band/utility-columns-footer` + `node v3/scripts/grammar-lint.js <composition.json>` + 시각 검토.
...
pending — 예약 경로: v3/renders/gold/footer-band--utility-columns-footer/

## 6. 출처

- recordId: `supabase`
- span order: `20`
- sectionSpan: `captures/stage2/supabase/tile_04.png#footer-band`
