...
주장과 설명을 먼저 읽고, 이어지는 큰 미디어 frame에서 보조 맥락을 검토하게 하는 세로 흐름이다. split은 두 영역을 동등 경쟁시키므로, 프레임을 두 번째 읽기 단계로 두는 이 변형에서는 배제했다. 카탈로그에서 frame 우선 hero 역할을 맡는다.

출처 타일에서 확인한 범위: 상단의 넓은 주장 영역 뒤에 독립된 인터페이스형 미디어 frame이 이어지는 opening 구조를 관찰했다. 기본값은 `stack` · `contrast-bleed`이며, 원본의 카피·브랜드·픽셀 값을 옮기지 않고 슬롯과 semantic 토큰으로 일반화했다.
...
- (c) 다른 모듈: 없음(독립). 조성 시 같은 `hero`·`stack` 조합은 한 번만 둘 수 있다.
...
- `layoutArchetype` 또는 `backgroundBleed` 기본값을 바꾸면 → 해당 CSS의 grid/표면 관계와 조성의 인접 다양성·이음새 규칙을 함께 바꿔야 한다. 재검증: `node v3/scripts/section-validate.js v3/modules/hero/framed-stack-hero` + `node v3/scripts/grammar-lint.js <composition.json>` + 시각 검토.
...
pending — 예약 경로: v3/renders/gold/hero--framed-stack-hero/

## 6. 출처

- recordId: `linear`
- span order: `2`
- sectionSpan: `captures/app-ui/linear/tile_00.png#hero`
