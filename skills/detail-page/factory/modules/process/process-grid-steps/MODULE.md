...
단계를 동등한 grid 항목으로 보여 주되 조립 입력 순서는 바꾸지 않는다.

프로파일 해부 앵커: `process-title` · `ordered-steps` · `next-action`. process 스팬의 희소성을 구조 근거로 과장하지 않고 SECTION-SCHEMA의 순서화된 단계 계약만으로 작성한다. 기본값은 `grid` · `card-contained`이며, 슬롯과 semantic 토큰으로만 일반화한다.
...
- 슬롯을 바꾸면 → 필수 슬롯 `headline`, `processSteps`와 `module.json`, template 앵커, SECTION-SCHEMA를 함께 검토해야 한다. 재검증: `node v3/scripts/section-validate.js v3/modules/process/process-grid-steps` + `node v3/scripts/grammar-lint.js <composition.json>` + 시각 검토.
...
pending — 예약 경로: v3/renders/gold/process--process-grid-steps/
