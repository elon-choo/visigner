...
질문과 답변을 세로 목록으로 쌓아 한 질문의 해소가 다음 질문의 전제가 되지 않게 한다. grid는 짧은 답변을 동등 카드처럼 오해하게 하므로 배제했다.

프로파일 해부 앵커: `faq-title` · `question-answer-set` · `unresolved-contact-route`. 허용 세그먼트의 high-confidence faq 분류를 구조 근거로 사용하고, 원본의 질문·답변·브랜드 요소는 일반화하지 않았다. 기본값은 `stack` · `surface-contained`이며, 슬롯과 semantic 토큰으로만 일반화한다.
...
- 슬롯을 바꾸면 → 필수 슬롯 `headline`, `faqItems`와 `module.json`, template 앵커, SECTION-SCHEMA를 함께 검토해야 한다. 재검증: `node v3/scripts/section-validate.js v3/modules/faq/faq-disclosure-stack` + `node v3/scripts/grammar-lint.js <composition.json>` + 시각 검토.
...
pending — 예약 경로: v3/renders/gold/faq--faq-disclosure-stack/
