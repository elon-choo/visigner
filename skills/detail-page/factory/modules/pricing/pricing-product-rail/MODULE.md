# pricing-product-rail

타입: `pricing` · 규격: SECTION-SCHEMA `1.0.0` · 게이트: `section-motion-v1`

## 1. 왜 이 방식인가

플랜을 개별 검토 폭이 있는 rail로 두어 조건을 서로 독립적으로 읽게 한다. 가격이나 브랜드 표현은 채굴하지 않는다.

프로파일 해부 앵커: `pricing-title` · `plan-set` · `terms-or-conditions`. pricing 구조와 styles inventory를 함께 가진 적격 소스는 posthog 하나뿐이다. 그 실제 구조 앵커를 유지하고 남은 변형 축만 계약 안에서 조합한다. 기본값은 `rail` · `surface-bleed`이며, 슬롯과 semantic 토큰으로만 일반화한다.
...
- 슬롯을 바꾸면 → 필수 슬롯 `headline`, `pricePlans`와 `module.json`, template 앵커, SECTION-SCHEMA를 함께 검토해야 한다. 재검증: `node v3/scripts/section-validate.js v3/modules/pricing/pricing-product-rail` + `node v3/scripts/grammar-lint.js <composition.json>` + 시각 검토.
...
pending — 예약 경로: v3/renders/gold/pricing--pricing-product-rail/
