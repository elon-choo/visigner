...
플랜을 세로 순서로 정리하되 각 조건을 가까운 카드 안에 남긴다. 시각적 순서는 계약 축으로만 추가했고 가격을 추정하지 않는다.

프로파일 해부 앵커: `pricing-title` · `plan-set` · `terms-or-conditions`. pricing 구조와 styles inventory를 함께 가진 적격 소스는 posthog 하나뿐이다. 그 실제 구조 앵커를 유지하고 남은 변형 축만 계약 안에서 조합한다. 기본값은 `stack` · `media-bleed`이며, 슬롯과 semantic 토큰으로만 일반화한다.
...
- 슬롯을 바꾸면 → 필수 슬롯 `headline`, `pricePlans`와 `module.json`, template 앵커, SECTION-SCHEMA를 함께 검토해야 한다. 재검증: `node v3/scripts/section-validate.js v3/modules/pricing/pricing-editorial-stack` + `node v3/scripts/grammar-lint.js <composition.json>` + 시각 검토.
...
pending — 예약 경로: v3/renders/gold/pricing--pricing-editorial-stack/
