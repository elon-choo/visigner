...
try {
  ...
  if (emitTheme) {
    ...
    for (const t of base) {
      // :root defines --brand-<path> for every token (alias or not), so the @theme map always points there
    ...
    for (const t of base) {
      const val = isAlias(t.token) ? `var(${brandVar(refOf(t.token))})` : leafValue(t.token);
      lines.push(decl(brandVar(t.path), val));
      emittedCount++;
    }
    lines.push('}');
