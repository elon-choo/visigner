...
- **`would_feature` is consumed, not decorative.** When the legibility precondition sets `would_feature=false` (a section renders faded/ghosted — a mechanical render bug), the taste gate does NOT pass: route it to **fix-and-reshoot** (a MECHANICAL fail per `review-rubric.md`, not a terminal taste reject), then re-grade the clean tiles. `would_feature=false` for any other reason (you genuinely would not put the page in front of the editorial panel) blocks the taste gate as a real reject. Either way `taste_gate_pass` cannot be true while `would_feature` is false.
