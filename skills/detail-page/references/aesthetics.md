...
- **COLOR COMMITMENT (mandatory — decide this before code, not after grading):** name the ANCHOR section that floods a *saturated color field* (a whole band owning the viewport, type reversed out of it) AND the positions that hold that color across the rest of the scroll (tinted grounds, repeated band backgrounds, or a dominant colored surface — not hairlines/chips/numerals, which the gate reads as quarantined), plus the risky-but-grounded pairing it commits to — **impact, not harmony.** A neutral base with a single accent confined to the CTA is the timid default this step exists to prevent: "no color" is not restraint, it reads as fear. (See `color-forward-palettes.md` for named impact-not-harmony palettes; a single-hue/monochrome scheme is a supporting band, never the page's identity.)
- **PRODUCT-VISUAL LANGUAGE (mandatory) — and for MATERIAL goods, show the material:** declare how the product will be shown. Gradient blobs are forbidden (see banned defaults). Decide this up front so image slots aren't backfilled with defaults. The rule is **subject-conditional**:
  - **Physical / material subject whose desirability IS its surface** (metal, wood, leather, ceramic, fabric, stone, food): at least ONE real **material image** is *required* — the actual surface, its light, its texture, its color/patina — generated via §4.5 asset-gen or a genuine photographic treatment. A committed illustration/diagram language (line-blueprint, exploded view) may **complement** it but must NOT fully substitute for it. Diagramming a brass dripper's geometry while never once showing the brass is a hole: the surface is the sell.
  - **Abstract / digital subject** (software, course, service, data): one committed illustration/diagram/screenshot system applied consistently is sufficient — there is no physical surface to photograph.
  - **Hybrid subjects** (cosmetics sold on results, a cooking class, a service that ships a physical good): apply the material-image requirement to the physical/sensory component AND a committed illustration/screenshot system to the digital/service component. Tiebreak: if the buyer physically receives or sensorially experiences ANY artifact, treat it as material and show that surface.
  - Either way, avoid thin line-art floated on a gradient as the *only* product visual: it reads as an empty placeholder on any subject that isn't already geometrically photogenic (see the density/focal floor in `review-rubric.md`).
