...
- [ ] **No empty placeholder panels (with a density/focal floor)** — a product/photo slot that renders as a **flat CSS gradient or solid panel with only a corner label** (no real photographic detail, no committed illustration/diagram, no focal subject) reads as *unfinished*, not designed. It is a **tangibility fail** (§B Visual-craft and — detail mode — Tangibility cannot score ≥7 while the product is never actually shown). Fix by either generating real assets (SKILL §4.5 `gen-assets.js`, free ChatGPT-OAuth path) for the hero + key product slots, OR making each CSS panel read as *finished* — a duotone-tinted macro texture with an actual focal subject and depth, never a bare gradient rectangle. "assetsOk=true" does NOT clear this: an empty gradient panel loads fine yet still shows nothing.
  - **Density/focal floor (don't let thin line-art pass the binary check):** "committed illustration" clears this ONLY if it has genuine illustration density AND a clear focal subject that renders the product. A few thin dashed strokes floated on a gradient with a mono label passes on a geometrically photogenic subject (a cone dripper) but reads *empty* on a non-geometric one (soft food, a service, a digital course). Judge whether the panel actually SHOWS something a viewer would recognize as the product, not whether it technically contains SVG.
  - **Material-subject clause:** for a physical subject whose desirability is its surface (metal/wood/leather/ceramic/fabric/food), a line-blueprint that never shows the actual **material** (its light, texture, color/patina) caps Visual-craft/Tangibility at 7 — the surface is the sell, so at least one real material image must appear (illustration may complement, not substitute). This is the #1 residual finish gap once the color and placeholder gaps are closed.
...
| **Visual craft** | Tiny mobile type, walls of text, flat; product shown only as flat gradient/placeholder panels | Clean bands, mobile-legible, GIF/video demos, dividers, intentional hierarchy; the product is actually SHOWN (real/macro photography or a committed illustration system), not stood in for by empty gradient panels |
...
**Color-confidence floor on Aesthetic distinctiveness (mandatory).** A palette that is **single-hue / monochrome**, OR a **neutral base with the accent confined to CTAs/highlights**, **caps Aesthetic distinctiveness at 6** — *regardless of how strong the signature element is.* A colorless signature does not clear the 8 ship floor.

**One quarantined band is NOT enough — the committed color must be STRUCTURAL, not fenced off.** A page whose committed color floods exactly one band while the *rest reverts to warm/cool-neutral monochrome* (color surviving elsewhere only as tiny section-number chips / hairlines) is the **"color-quarantined" trap** and **caps Aesthetic distinctiveness at 7, not 8.** To reach 8, the committed color must recur as a **load-bearing element across a meaningful share of the scroll** — full-bleed section grounds, repeated band backgrounds, or a dominant surface in several sections — so the page *reads* color-confident top-to-bottom, not brown-on-brown with one fenced-off hue. Map to the taste jury's **Color-confidence** axis (`taste-jury.md`) and grade with its threshold: a single quarantined band scores taste color ≈6, so anti-slop distinctiveness must not exceed 7 for the same pixels — the two panels must not diverge on this axis. (Historical failures this closes: (1) a warm-amber-monochrome page scored distinctiveness 8 / ship-pass here while taste scored color 4.5; (2) a page with ONE verdigris band across 15 warm-neutral sections scored distinctiveness 8 here while taste scored color 6 — the single-band escape hatch is now closed.)
