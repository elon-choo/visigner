...
node ${CLAUDE_PLUGIN_ROOT}/skills/detail-page/scripts/copy-lint.js <spec.json|dir/> [out.json] [--idea "the one promise"] [--strict] [--locale ko] [--voice voice.json] [--lexicon voice.json]
```
...
- **message-match is a PRESENCE-ONLY floor, not a semantic judge.** With `--idea`, the default is a **WARN** that flags any surface whose hero shares ZERO *content-tokens* with the pinned idea (token presence only). **`--strict` promotes it to an ERROR / gate-fail** on that same zero-overlap condition. It does NOT grade whether the surface *means* the idea — semantic/meaning match stays **design-critic `MODE=copy`**'s job. **Caveat:** a single-char/trivial idea (e.g. literal `x`) yields no content-tokens (en path needs len ≥4, ko path len ≥2), so the floor no-ops on empty/trivial ideas by design — pass a real idea phrase.
- **Use it for the `/campaign` ladder** — pin ONE idea, build a per-surface ladder (landing + ad + social + an email *sequence*; each with message / awareness / angle / metric), then run **`copy-lint.js --idea`** for the cross-surface *presence* floor (add `--strict` to gate on zero overlap) + **`email-lint.js`** for the sequence + **design-critic `MODE=copy`** for the *semantic* message-match + voice grade. That whole flow is the **`/campaign`** command (`commands/campaign.md`); reach for it on "campaign / 캠페인 / 런치 캠페인 짜줘 / one idea across channels / 메시지 일관성". The §Sequences launch-runway arc is the email half of that ladder.
