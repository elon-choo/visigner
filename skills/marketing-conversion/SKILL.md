...
node ${CLAUDE_PLUGIN_ROOT}/skills/detail-page/scripts/copy-lint.js <spec.json|dir/> [out.json] [--idea "the one promise"] [--strict] [--voice-fingerprint] [--locale ko] [--voice voice.json] [--lexicon voice.json]
```
...
- **message-match is a PRESENCE-ONLY floor, not a semantic judge.** With `--idea`, message-match is **advisory WARN-only**: it flags a surface whose hero shares ZERO *content-tokens* with the pinned idea, but treats an **in-voice paraphrase that shares stems OR owned brand-terms** as a correct restatement (not drift), so a legit reworded promise no longer trips it. `--lexicon` `owned[]` terms now **augment** the `--idea` token set, so brand vocabulary counts as on-message. It does NOT grade whether the surface *means* the idea — semantic/meaning match stays **design-critic `MODE=copy`**'s job. **Caveat:** a single-char/trivial idea (e.g. literal `x`) yields no content-tokens (en path needs len ≥4, ko path len ≥2), so the floor no-ops on empty/trivial ideas by design — pass a real idea phrase.
