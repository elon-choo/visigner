...
```bash
# SIZE a continuous metric (revenue / AOV / time-on-task) BEFORE you run it — the proportion `plan` is wrong
# for a mean. n = 2·(z_{1-α/2}+z_{1-β})²·σ²/δ² per variant, with δ = μ·mde; same --daily duration estimate
# and >4-week warning as the proportion plan.
node ${CLAUDE_PLUGIN_ROOT}/skills/detail-page/scripts/ab.js plan --metric mean --mu 52.10 --sd 38.0 --mde 0.05 [--power 0.8] [--alpha 0.05] [--daily 1200] [--variants 2]
...
# PEEK-SAFE sequential readout (mixture SPRT, always-valid): lets a marketer check at ANY time and get a valid
# stop/continue decision — no fixed-horizon, no peeking penalty. Reports an always-valid p (≥ the fixed-horizon
# `test` p) and STOPs for significance via Ville's inequality (Λ ≥ 1/α). τ (prior SD on the true absolute diff)
# MUST be pre-committed from the MDE — defaults to |baseline·mde| if both given, else 0.01 (1pp); override with --tau.
# (Robbins 1970 + Johari–Pekelis–Walsh 2017 "Always Valid Inference", the Optimizely Stats Engine basis.)
node ${CLAUDE_PLUGIN_ROOT}/skills/detail-page/scripts/ab.js seq --a 120/4000 --b 156/4100 [--baseline 0.03 --mde 0.10 | --tau 0.01]
# PAID-CHANNEL sanity (no experiment needed): CAC = spend/conv, ROAS = revenue/spend, AOV = revenue/conv. With
# --margin it adds profit-ROAS (revenue·margin/spend), breakeven ROAS (1/margin), net-after-ad, and a
# PROFITABLE / UNPROFITABLE verdict.
node ${CLAUDE_PLUGIN_ROOT}/skills/detail-page/scripts/ab.js roas --spend 5000 --conv 120 --revenue 18000 [--margin 0.6]
```
...
node ${CLAUDE_PLUGIN_ROOT}/skills/detail-page/scripts/email-lint.js <spec.json|email.html|dir/> [--locale ko] [--voice voice.json] [--lexicon voice.json] [out.json]
```
- **Locale floor (NEW):** `--locale ko` swaps in a built-in Korean pack — Korean slop/superlative + Korean spam tells + Korean CTA verbs, and Korean-aware subject/preview budgets (25/45 graphemes vs en 50/90). `--voice voice.json` (`{locale, slopWords, spamWords, ctaVerbs, subjectMax, previewMax}`) overrides individual fields for a custom locale/voice. Default is exact English behavior; **non-English locales count length in graphemes** (a Korean syllable = 1, so a 25-char KR subject isn't penalized as 50+ UTF-16 units). **This `--voice` locale pack is a DIFFERENT file from `--lexicon voice.json`** (the `{owned,banned}` brand-word list shared with `copy-lint`/`brand-lint`) — `--voice` tunes the language floor, `--lexicon` enforces brand vocabulary; don't conflate the two shapes.
...
- **email-lint severities:** ERROR (exit 1) = subject missing / >50 chars, zero CTAs, ALL-CAPS or `!!!` in subject/preheader, a **true** spam tell (rule `spam-words`: *free money, act now, cash bonus, no cost, 100% free, winner, congratulations you, this is not spam, extra income, click here, risk-free*), any `--lexicon` banned term, **and an AI-slop banned verb anywhere in subject + preheader + body** (rule `slop-verb` — the same SLOP list `copy-lint.js` enforces: *Empower / Unlock / Transform / Build the future / Revolutionize / Supercharge / Unleash / Reimagine / Elevate / Disrupt / game-changer / next-level / cutting-edge*; one finding per distinct match, deduped). So a slop-laden email **body** now fails the gate, matching the suite's copy convention — not just the subject line. WARN (never fails) = no preheader / >90 chars, >1 CTA, >2 emoji, none of the owned terms present, and a **borderline-but-legit launch phrase** (new rule `spam-words-soft`: *guarantee, order now, buy now, limited time, urgent*) — these are downgraded from ERROR so real launch copy no longer fails the gate. **Sequence/dir mode dedupes a spec+HTML pair:** a `welcome.json` + `welcome.html` in the same dir count **once** by basename (the `.json` spec is preferred when both exist), so a built sequence isn't double-linted. Run it as the gate before handing the copy to **design-critic `MODE=copy`** for the voice/consistency read — deterministic checks first, LLM judgment second.
- **One canonical banned-term list (SSOT):** `email-lint.js` and `copy-lint.js` both import `${CLAUDE_PLUGIN_ROOT}/skills/detail-page/scripts/lib-banned-words.js` — the single source for the AI-slop/superlative list (`EN_BANNED`: 19 RegExp inflection families with word boundaries, at parity with the SKILL banned-defaults — incl. *seamless / revolutionary / innovative / best-in-class / world-class / game-changing / cutting-edge / next-gen / supercharge / elevate* atop *empower / unlock / transform / unleash / reimagine / disrupt / next-level*; `KO_BANNED`: 20 substrings incl. *혁신적 / 세계 최초*), plus `findBanned(corpus,list)` and `bannedFor(locale)`. The former per-script `SLOP`/`KO_SLOP` arrays and `findSlop` helpers are gone (`SLOP=EN_BANNED`, `KO_SLOP=KO_BANNED` aliases retained for the locale table). **Extending the banned list now means editing only that one file**; no CLI/flag/exit-code behavior changed.
...
node ${CLAUDE_PLUGIN_ROOT}/skills/detail-page/scripts/copy-lint.js <spec.json|dir/> [out.json] [--idea "the one promise"] [--synonyms syn.json] [--strict] [--voice-fingerprint] [--locale ko] [--voice voice.json] [--lexicon voice.json]
```
...
- **Checks:** the per-channel length floor, an AI-slop **banned-verb** list (Empower / Unlock / Transform / Build the future / …), a **required CTA**, an optional owned/banned `--lexicon voice.json` (shared with email-lint), and — with `--idea` — the cross-surface **message-match floor** (next bullet).
- **Locale floor (NEW):** `--locale ko` swaps in the built-in Korean pack (Korean slop/superlative + Korean CTA verbs); `--voice voice.json` (`{locale, slopWords, spamWords, ctaVerbs, subjectMax, previewMax}`) overrides individual fields. Non-English locales count length in **graphemes** (Korean syllable = 1). This `--voice` locale pack is **distinct from `--lexicon voice.json`** (`{owned,banned}` brand words) — different file shapes, don't conflate them.
- **message-match is a PRESENCE-ONLY floor, not a semantic judge.** With `--idea`, message-match is **advisory WARN-only**: it now scans the **WHOLE surface copy** (hero + primary + subhead/body), not just the hero/first line — so a hook-first surface that restates the idea in the body is no longer false-WARNed — and flags only a surface sharing ZERO *content-tokens* with the pinned idea. It treats an **in-voice paraphrase that shares stems OR owned brand-terms** as a correct restatement (not drift), so a legit reworded promise no longer trips it. Two ways to teach it your paraphrases: `--lexicon` `owned[]` terms (and a `voice.json` `owned` list) **augment** the `--idea` token set, and the new **`--synonyms <path.json>`** flag folds extra terms into the idea stems — accepting either an array `["rest","slumber"]` or a map `{"sleep":["rest","slumber"]}` (the keys count too). It does NOT grade whether the surface *means* the idea — semantic/meaning match stays **design-critic `MODE=copy`**'s job. **Caveats:** a single-char/trivial idea (e.g. literal `x`) yields no content-tokens (en path needs len ≥4, ko path len ≥2), so the floor no-ops on empty/trivial ideas by design — pass a real idea phrase; and Korean 2-char synonyms match only standalone token forms, not josa-attached ones (`숙면` won't match `숙면을`, the KO ≥4-char prefix rule) — list KO synonyms in josa-stripped form.
- **`--strict` now enforces VERBATIM TAGLINE/CLAIM LOCKS ONLY** (not message-match — that stays advisory above). It ERRORs / gate-fails when a surface-declared **lock string is absent** from that surface's copy. Declare locks per-surface with `locks` / `verbatimLocks` / `verbatim` (a string or an array of exact strings that must appear word-for-word — e.g. the legal tagline or a substantiated claim). Use it to guarantee the approved wording shipped intact across the ladder; semantic restatement is fine elsewhere, but a locked line must be exact.
- **`--voice-fingerprint` (alias `--fingerprint`) — deterministic advisory register pass, never fails the gate.** It WARNs on surfaces that are **register outliers** vs the campaign baseline across three signals: owned-term coverage, sentence-length variance, and Flesch-Kincaid reading grade — catching a surface that drifts in voice even when message and locks are fine. It needs **≥3 surfaces** to form a baseline (fewer = no-op). Shared lib `scripts/lib-voice-fingerprint.js` powers BOTH `copy-lint.js` and `email-lint.js`, so the ad/social/push ladder and the email sequence are graded on one register model. It's advisory only — the semantic voice/consistency verdict still belongs to **design-critic `MODE=copy`**.
- **Use it for the `/campaign` ladder** — pin ONE idea, build a per-surface ladder (landing + ad + social + an email *sequence*; each with message / awareness / angle / metric), then run **`copy-lint.js --idea`** for the cross-surface *presence/paraphrase* floor (add `--strict` to gate on absent verbatim tagline/claim locks, `--voice-fingerprint` for the register-outlier advisory once the ladder has ≥3 surfaces) + **`email-lint.js`** for the sequence + **design-critic `MODE=copy`** for the *semantic* message-match + voice grade. That whole flow is the **`/campaign`** command (`commands/campaign.md`); reach for it on "campaign / 캠페인 / 런치 캠페인 짜줘 / one idea across channels / 메시지 일관성". The §Sequences launch-runway arc is the email half of that ladder.
