...
4. **Ship the logo package — crops, not just the master. One command does it: `logo-handoff.js`.** From the one `mark.svg`, `scripts/logo-handoff.js` emits the whole artboard SET — `full-color.svg`, `mono.svg` (1-color), `knockout.svg` (white-on-dark), `favicon.svg` (32px), `app-icon.svg` (512px, rounded/padded), `social-avatar.svg` (512px circle crop) — plus an `index.html` that inlines all six (it feeds the logo-grid sheet). Every board **nests the SAME source mark** (scaled via `preserveAspectRatio`, never redrawn) with clear-space + min-size baked in; mono/knockout are derived deterministically with an SVG `feColorMatrix` (any input color → a single-color silhouette preserving alpha), so they hold on any mark without re-authoring fills:
   ...
   ```
   **It AUTO-STRIPS construction guides before nesting the source mark** — so you can hand it a curated on-grid starter (the `logo-archetypes/` marks that wrap guides in `<g id="construction-grid">`) without manually deleting them first. It removes the `<g id="construction-grid">` and `<g id="clear-space">` groups (balanced `<g>` matching), XML comments, the source `<title>`/`<desc>` (which carried the "delete #construction-grid" instruction text), and any self-closing element with a `data-guide` attribute; then it **re-scans**. If any guide marker survives (residual `id="construction-grid|clear-space"`, `data-guide`, or `stroke-dasharray` — e.g. a dashed guide path living OUTSIDE the known groups, or an unbalanced group), it **HARD-REFUSES with exit code 3** and `GUIDE-FAIL: guides present — strip before handoff`, writing NO artboards (mirroring `design-critic MODE=logo`'s auto-FAIL on leaked guides). **Caveat:** a legitimate mark that intentionally uses `stroke-dasharray` is refused by design — strip/flatten the dashes first. (The boards' own intentional clear-space dashed overlay is drawn by `board()` with `guides:true` and is unrelated to source guides; clean marks pass through visually unchanged.) Re-shoot the emitted `index.html` to verify the set. (For a hand-built bespoke crop, the manual targets remain: favicon 16/32/48 `.ico`, app-icon 1024px with maskable padding, circular social-avatar — each min-size-tested in the sheet above.)
